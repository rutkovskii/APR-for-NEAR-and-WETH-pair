...
BRL_id = 'borealis'
BRL_price = get_price(BRL_id)

# NEAR-WETH pool info
pool_id = 1
pool_info = contract.functions.poolInfo(pool_id).call()
totalAllocPoint = contract.functions.totalAllocPoint().call()
perc_of_rewards = pool_info[1] / totalAllocPoint
print(f"Pool {pool_id} alloc points = {pool_info[1]} = {perc_of_rewards*100:.2f}% of rewards")

# rewards per year
rewards_per_year_vfat = BRLPerBlock/1e18 * BRL_price * perc_of_rewards *(60*60*24/ 1.1) * 365
rewards_per_year = BRLPerBlock/1e18 * BRL_price * perc_of_rewards * blocks_per_day() * 365
...
lp_contract = w3.eth.contract(pool_info[0], abi=LP_ABI)

# Now get the balance on the masterchef
lp_staked = lp_contract.functions.balanceOf(address).call()/1e18
print(f'lp_staked\t\t{lp_staked:.4f} Aurora-LP')

# Now for a general solution this is trickier but if you only need this one pool then I would just get the ETH/lp token. then multiply by the price of ETH.
# The lp token is NEAR/WETH so WETH is the 2nd token on the reserves
#print(lp_contract.functions.getReserves().call())
eth_per_lp = lp_contract.functions.getReserves().call()[1] / lp_contract.functions.totalSupply().call()
#print(f'eth_per_lp\t\t{eth_per_lp}')

ETH_id = 'ethereum'
ETH_price = get_price(ETH_id)

# So each lp token is worth that much ETH + an equivalent amount of NEAR
...
APR_vfat = rewards_per_year_vfat/staked_value_in_usd * 100
